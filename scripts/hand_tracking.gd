...
func getFingersGripping(palm_area):
	...
	for area in areas:
		if "Tip" in area.get_name():
			fingers_gripping += 1
	# get_node("../../OutputNode/Viewport/OtherLabel").text = fingers_gripping as String
	return fingers_gripping

func _on_Palm_Area_area_entered(area):
	var palm_area = $HandModel/Armature/Skeleton/Palm/Palm_Area
	if getFingersGripping(palm_area) > 0 and not held_object:
		get_node("../../OutputNode/Viewport/GripLabel").text = "Hand Gripping"
		grab_object()
